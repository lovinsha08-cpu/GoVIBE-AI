...
export async function generate(req, res, next) {
  try {
    ...
    const { data: saved, error: saveError } = await supabaseAdmin
      ...
      .insert({
        ...
        budget_summary: {
          ...
          ai_extras: { ...(result.budgetSummary?.ai_extras || {}), journey: result.journey },
        },
    ...
    res.status(201).json({ itinerary: saved, hiddenGems: result.hiddenGems });
  } catch (err) {
    next(err);
  }
}

// POST /api/itinerary/:tripId/stop/:stopOrder/regenerate
...
// GET /api/itinerary/:tripId/latest
export async function getLatest(req, res, next) {
  try {
    const { data, error } = await supabaseAdmin
      .from('itineraries')
      .select('*, trips!inner(traveler_id)')
      .eq('trip_id', req.params.tripId)
      .eq('trips.traveler_id', req.user.id)
      .order('version', { ascending: false })
      .limit(1)
      .single();

    if (error) return res.status(404).json({ error: 'No itinerary found for this trip' });
...
// GET /api/itinerary/:tripId/download — streams a branded PDF of the
// latest saved itinerary for this trip.
export async function downloadPdf(req, res, next) {
  try {
    ...
    const pdfBuffer = await buildItineraryPdfBuffer({
      trip: { ...trip, traveler_name: traveler?.full_name || null },
      itinerary,
    });

    const safeDestination = (trip.destination || 'itinerary').replace(/[^a-z0-9]+/gi, '-');
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="GoVIBE-${safeDestination}.pdf"`);
